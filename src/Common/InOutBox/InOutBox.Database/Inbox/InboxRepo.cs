...
{
    public class InboxRepo<TDbContext>(TDbContext context) : IInboxRepo
        where TDbContext : DbContext, IInboxDbContext
    {
        public Task AddRangeAsync(IReadOnlyCollection<IDomainEvent> events, CancellationToken cancellationToken)
            => context.AddManyWithinTransaction<InboxEntity, TDbContext>(async dbContext =>
            {
                var addedEntries = new List<EntityEntry<InboxEntity>>();

                foreach (var item in events.Select(InboxEntity.From))
                {
        ...
        public async Task<ReadOnlyCollection<InOutboxEvent>> ClaimPendingAndFailedEventsAsync(int batchSize, CancellationToken cancellationToken)
            ...
            {
                var pendingEvents = dbContext.InboxEvents
                    .Where(e =>
        ...
        {
            var @event = await context.InboxEvents.SingleAsync(e => e.Id == eventId, cancellationToken);

            @event.Status = InOutboxStatus.Done;
            @event.ProcessedAt = DateTime.UtcNow;


            context.InboxEvents.Update(@event);
