...
using Stock.Seed.Models.FileEntity;
...
public static class SeedExtensions
{
    ...
    public static async Task ApplySeed(this IServiceProvider serviceProvider, IHostEnvironment env)
    {
        ...
        var seed = _yamlDeserializer.Deserialize<SeedModel>(await File.ReadAllTextAsync(seedPath));

        foreach (var file in seed.Files.OrderBy(x => x.Order))
        {
            var splitted = file.Path.Split('/');
            var memberPath = Path.Combine(_basePath, Path.Combine(splitted));
            ...
            if (file.IsProcedure)
            {
                await db.Database.ExecuteSqlRawAsync(await File.ReadAllTextAsync(memberPath));
                continue;
            }
            ...
            if (member.Metadata.AdditionalUseCaseName != null && member.Metadata.AdditionalUseCaseName == "FilesUseCase")
            {
                ...
                member = ConvertToSeedMember(seedMemberFile.Metadata, seedMemberFile.Data);
            }
    ...
    internal static SeedMember ConvertToSeedMember(SeedMemberMetadata meta, List<SeedFileData>? data)
    {
        SeedFileData s;
...
#pragma warning restore CS8604 // Possible null reference argument.
    ...
    internal static string GenerateInsertStatements(this SeedMember seedMember)
    {
